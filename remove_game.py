...
class RemovePlayer:
    def __init__(self, board, board_bit):
        init_game = NoTippingRemoveGame(board, board_bit)
        self.state = RemoveGameState(init_game, Player.BLACK)
        self.cache = {}
...
if __name__ == '__main__':
    ...
    init_game = NoTippingRemoveGame(state, np.uint64(2305840810190503935))
    state = RemoveGameState(init_game, Player.BLACK)
    start = time.time()
    print(Max(state, -1, 1, {}))
    print(time.time() - start)